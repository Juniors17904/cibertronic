...
// Realiza la solicitud para agregar el usuario
document.getElementById('userForm').addEventListener('submit', function (event) {
    event.preventDefault();  // Previene el envío tradicional del formulario
    console.log('Datos del formulario:', Object.fromEntries(formData)); // ✔️ Correcto

    fetch('/cybertro/controllers/create_user.php', {
        method: 'POST',  // o 'GET', dependiendo de tu solicitud
        body: new FormData(this)  // Usamos 'this' para obtener el formulario que disparó el evento
    })
        .then(response => response.json())  // Convertimos la respuesta JSON
        .then(data => {
            if (data.success) {
                showModal(data.message);  // Si es exitoso, mostramos el mensaje
            } else {
                showModal('Error al crear el usuario.');  // Si hay error
            }
        })
        .catch(error => {
            showModal('Error: ' + error.message);  // Muestra el mensaje de error
        });
});

// Función para mostrar el modal con el mensaje
function showModal(message) {
    document.getElementById('modalMessage').textContent = message;  // Inserta el mensaje en el modal
    document.getElementById('confirmationModal').style.display = 'block';  // Muestra el modal
}

// Función para cerrar el modal
function closeModal() {
    document.getElementById('confirmationModal').style.display = 'none';  // Oculta el modal
}
